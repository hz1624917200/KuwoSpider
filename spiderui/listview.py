...
class ListView(QWidget):  # 列表类
	...
	def download(self, qModelIndex):  # 从歌手列表或榜单列表下载歌曲
		...
		if exists(full_name):
			...
			if choice == QMessageBox.Yes:
				...
				QMessageBox.information(self, "下载提示",
				                        song_name + '下载成功！下载至' + self.path + song_name + '(' + str(i) + ')' + '.mp3')
		else:
			try:
	...
	def get_ranksong(self, qModelIndex):  # 获取所有榜单
		try:
			...
			ListView.Ranksong_List = Network.search_by_list(
				self.rank_list[self.qList.index(self.qList[qModelIndex.row()])], page.PageChange.flag)
